...
const w3cSlidy = {
  ...
  ie: (typeof document.getElementsByTagName('*') !== 'undefined' && !this.opera),
  ...
  // attach initialization event handlers
  set_up: function () {
    const init = function () { w3cSlidy.init() }
  ...
  hide_slides: function () {
    if (document.body && !w3cSlidy.initialized) {
      document.body.style.visibility = 'hidden'
    } else { setTimeout(w3cSlidy.hide_slides, 50) }
  },
  ...
  init: function () {
    ...
    if (this.keyboardless) {
      w3cSlidy.remove_class(w3cSlidy.toolbar, 'hidden')
      this.want_toolbar = 0
    }

    // work around for opera bug
    this.is_xhtml = !!(document.body.tagName === 'BODY')

    if (this.slides.length > 0) {
      const slide = this.slides[this.slide_number]
    ...
    if (this.ie7) { setTimeout(w3cSlidy.ie_hack, 100) }

    this.show_toolbar()

    // for back button detection
    setInterval(function () { w3cSlidy.check_location() }, 200)
    w3cSlidy.initialized = true
  },

  // create div element with links to each slide
  table_of_contents: function () {
    const toc = this.create_element('div')
    this.add_class(toc, 'slidy_toc hidden')
    // toc.setAttribute('tabindex', '0');

    const heading = this.create_element('div')
    ...
    for (let i = 0; i < this.slides.length; ++i) {
      const title = this.has_class(this.slides[i], 'title')
      const num = document.createTextNode((i + 1) + '. ')

      toc.appendChild(num)

      const a = this.create_element('a')
      a.setAttribute('href', '#(' + (i + 1) + ')')

      if (title) { this.add_class(a, 'titleslide') }

      const name = document.createTextNode(this.slide_name(i))
      a.appendChild(name)
      a.onclick = w3cSlidy.toc_click
      a.onkeydown = w3cSlidy.toc_key_down
      a.previous = previous
      ...
      if (i < this.slides.length - 1) {
        const br = this.create_element('br')
        toc.appendChild(br)
      }
    ...
    toc.onmouseup = w3cSlidy.mouse_button_up

    toc.onclick = function (e) {
      e || (e = window.event)

      if (w3cSlidy.selected_text_len <= 0) {
        w3cSlidy.hide_table_of_contents(true)
      }

      w3cSlidy.stop_propagation(e)
  ...
  is_shown_toc: function () {
    return !w3cSlidy.has_class(w3cSlidy.toc, 'hidden')
  },

  show_table_of_contents: function () {
    w3cSlidy.remove_class(w3cSlidy.toc, 'hidden')
    const toc = w3cSlidy.toc
    toc.focus()

    if (w3cSlidy.ie7 && w3cSlidy.slide_number === 0) {
      setTimeout(w3cSlidy.ie_hack, 100)
    }
  },

  hide_table_of_contents: function (focus) {
    w3cSlidy.add_class(w3cSlidy.toc, 'hidden')

    if (focus && !w3cSlidy.opera &&
      !w3cSlidy.has_class(w3cSlidy.toc, 'hidden')) {
      w3cSlidy.set_focus()
    }
  },

  toggle_table_of_contents: function () {
    if (w3cSlidy.is_shown_toc()) {
      w3cSlidy.hide_table_of_contents(true)
    } else { w3cSlidy.show_table_of_contents() }
  },
  ...
  toc_click: function (e) {
    ...
    const target = w3cSlidy.get_target(e)
    ...
    if (target && target.nodeType === 1) {
      ...
      if (uri) {
        // alert("going to " + uri);
        let slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)
        w3cSlidy.slide_number = w3cSlidy.find_slide_number(uri)
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.last_shown = null
        w3cSlidy.set_location()
        w3cSlidy.set_visibility_all_incremental('hidden')
        w3cSlidy.set_eos_status(!w3cSlidy.next_incremental_item(w3cSlidy.last_shown))
        w3cSlidy.show_slide(slide)
        // target.focus();

        try {
          if (!w3cSlidy.opera) { w3cSlidy.set_focus() }
        } catch (e) { }
      }
    }

    w3cSlidy.hide_table_of_contents(true)
    if (w3cSlidy.ie7) w3cSlidy.ie_hack()
    w3cSlidy.stop_propagation(e)
    return w3cSlidy.cancel(e)
  },
  ...
  toc_key_down: function (event) {
    ...
    if (window.event) {
      ...
      key = event.which
    } else { return true } // Yikes! unknown browser
    ...
    if (key === 13) {
      ...
      if (self.ie7) { w3cSlidy.ie_hack() }

      return w3cSlidy.cancel(event)
    }
  ...
  touchstart: function (e) {
    ...
    const tapDelay = this.last_tap - this.prev_tap
  ...
  touchend: function (e) {
    ...
    const delay = (new Date()).getTime() - this.last_tap
    const dx = this.delta_x
    const dy = this.delta_y
    const absDx = Math.abs(dx)
    const absDy = Math.abs(dy)

    if (delay < 500 && (absDx > 100 || absDy > 100)) {
      if (absDx > 0.5 * absDy) {
        e.preventDefault()

        if (dx < 0) {
          w3cSlidy.next_slide(true)
        } else {
          w3cSlidy.previous_slide(true)
        }
      } else if (absDy > 2 * absDx) {
        e.preventDefault()
        w3cSlidy.toggle_table_of_contents()
      }
    }
  },
  ...
  slide_name: function (index) {
    ...
    const heading = this.find_heading(slide)
  ...
  find_duration: function () {
    ...
    for (let i = 0; i < meta.length; ++i) {
      ...
      if (name === 'duration') { return 60000 * content }
    }
    ...
    for (let i = 0; i < items.length; ++i) {
      ...
      */
      if (this.foldable(target)) {
        ...
        target.onfocus = function () { w3cSlidy.outline = this }
        target.onblur = function () { w3cSlidy.outline = null }
  ...
  foldable: function (item) {
    ...
    while (node) {
      if (node.nodeType === 1 && this.is_block(node)) { return true }
  ...
  fold: function (item) {
    ...
    let node = item ? item.firstChild : null

    while (node) {
      if (node.nodeType === 1 && this.is_block(node)) { // element
        w3cSlidy.add_class(node, 'hidden')
      }
  ...
  unfold: function (item) {
    ...
    while (node) {
      if (node.nodeType === 1 && this.is_block(node)) { // element
        w3cSlidy.remove_class(node, 'hidden')
      }
  ...
  outline_click: function (e) {
    ...
    let target = w3cSlidy.get_target(e)
    ...
    if (e.which) {
      this.rightclick = (e.which === 3)
    } else if (e.button) {
      this.rightclick = (e.button === 2)
    }

    if (!this.rightclick && target.visible !== undefined) {
      if (target.foldable) {
        if (target.visible) {
          w3cSlidy.fold(target)
        } else {
          w3cSlidy.unfold(target)
        }
      }
  ...
  add_initial_prompt: function () {
    const prompt = this.create_element('div')
    prompt.setAttribute('class', 'initial_prompt')

    const p1 = this.create_element('p')
  ...
  add_toolbar: function () {
    ...
    if (this.ns_pos || !this.ie6) {
      const right = this.create_element('div')
      ...
      const left = this.create_element('div')
      ...
      const help = this.create_element('a')
      ...
      const gap1 = document.createTextNode(' ')
      left.appendChild(gap1)

      const contents = this.create_element('a')
      ...
      const gap2 = document.createTextNode(' ')
      left.appendChild(gap2)

      const copyright = this.find_copyright()
      ...
      const sp = this.create_element('span')
      sp.innerHTML = '&nbsp;&nbsp;*&nbsp;'
    ...
    for (let i = 0; i < headings.length; ++i) {
      ...
      while (node) {
        if (node.nodeType === 1) { // an element
          if (node.nodeName === 'H1' || node.nodeName === 'h1') { break }
  ...
  patch_anchors: function () {
    ...
    const handler = function (event) {
      ...
      if (self.page_address(this.href) === self.page_address(location.href)) {
        ...
        const newslidenum = self.find_slide_number(this.href)

        if (newslidenum !== self.slide_number) {
          let slide = self.slides[self.slide_number]
      ...
      } else { w3cSlidy.stop_propagation(event) }
  ...
  show_slide_number: function () {
    const timer = w3cSlidy.get_timer()
    w3cSlidy.slide_number_element.innerHTML = timer + w3cSlidy.localize('slide') + ' ' +
      (w3cSlidy.slide_number + 1) + '/' + w3cSlidy.slides.length
  },
  ...
  check_location: function () {
    ...
    if (w3cSlidy.slide_number > 0 && (hash === '' || hash === '#')) {
      w3cSlidy.goto_slide(0)
    } else if (hash.length > 2 && hash !== '#(' + (w3cSlidy.slide_number + 1) + ')') {
      const num = parseInt(location.hash.substr(2))

      if (!isNaN(num)) { w3cSlidy.goto_slide(num - 1) }
    }

    if (w3cSlidy.time_left && w3cSlidy.slide_number > 0) {
      w3cSlidy.show_slide_number()

      if (w3cSlidy.time_left > 0) { w3cSlidy.time_left -= 200 }
    }
  },

  get_timer: function () {
    let timer = ''
    if (w3cSlidy.time_left) {
      let secs = Math.floor(w3cSlidy.time_left / 1000)
  ...
  set_location: function () {
    let uri = w3cSlidy.page_address(location.href)
    let hash = '#(' + (w3cSlidy.slide_number + 1) + ')'

    if (w3cSlidy.slide_number >= 0) { uri = uri + hash }

    if (typeof (history.pushState) !== 'undefined') {
      document.title = w3cSlidy.title + ' (' + (w3cSlidy.slide_number + 1) + ')'
      history.pushState(0, document.title, hash)
      w3cSlidy.show_slide_number()
      w3cSlidy.notify_observers()
      return
    }

    if (w3cSlidy.ie && (w3cSlidy.ie6 || w3cSlidy.ie7)) { w3cSlidy.push_hash(hash) }

    if (uri !== location.href) { location.href = uri }// && !khtml

    if (this.khtml) { hash = '(' + (w3cSlidy.slide_number + 1) + ')' }
  ...
  on_frame_loaded: function (hash) {
    location.hash = hash
    const uri = w3cSlidy.page_address(location.href)
  ...
  push_hash: function (hash) {
    ...
    const doc = document.getElementById('historyFrame').contentWindow.document
  ...
  find_slide_number: function (uri) {
    ...
    const anchor = unescape(uri.substr(i + 1))
    ...
    if (!target) {
      ...
      if (anchor.match(re)) {
        let num = parseInt(anchor.substring(1, anchor.length - 1))
    ...
    while (true) {
      ...
      if (!target) { return 0 } // no luck!
    }

    for (let i = 0; i < this.slides.length; ++i) {
      if (this.slides[i] === target) { return i } // success
    }
  ...
  previous_slide: function (incremental) {
    if (!w3cSlidy.view_all) {
      let slide

      if ((incremental || w3cSlidy.slide_number === 0) && w3cSlidy.last_shown !== null) {
        w3cSlidy.last_shown = w3cSlidy.hide_previous_item(w3cSlidy.last_shown)
        w3cSlidy.set_eos_status(false)
      } else if (w3cSlidy.slide_number > 0) {
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)

        w3cSlidy.slide_number = w3cSlidy.slide_number - 1
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.set_visibility_all_incremental('visible')
        w3cSlidy.last_shown = w3cSlidy.previous_incremental_item(null)
        w3cSlidy.set_eos_status(true)
        w3cSlidy.show_slide(slide)
      }

      w3cSlidy.set_location()

      if (!w3cSlidy.ns_pos) { w3cSlidy.refresh_toolbar(200) }
    }
  },

  next_slide: function (incremental) {
    if (!w3cSlidy.view_all) {
      let slide = w3cSlidy.last_shown
      const last = w3cSlidy.last_shown

      if (incremental || w3cSlidy.slide_number === w3cSlidy.slides.length - 1) {
        w3cSlidy.last_shown = w3cSlidy.reveal_next_item(w3cSlidy.last_shown)
      }

      if ((!incremental || w3cSlidy.last_shown == null) &&
        w3cSlidy.slide_number < w3cSlidy.slides.length - 1) {
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)
        ...
        w3cSlidy.set_visibility_all_incremental('hidden')
        w3cSlidy.show_slide(slide)
      } else if (!w3cSlidy.last_shown) {
        if (last && incremental) { w3cSlidy.last_shown = last }
      }
  ...
  first_slide: function () {
    if (!w3cSlidy.view_all) {
      ...
      if (w3cSlidy.slide_number !== 0) {
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)

        w3cSlidy.slide_number = 0
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.last_shown = null
        w3cSlidy.set_visibility_all_incremental('hidden')
        w3cSlidy.show_slide(slide)
      ...
      w3cSlidy.set_eos_status(
        !w3cSlidy.next_incremental_item(w3cSlidy.last_shown))
      w3cSlidy.set_location()
    }
  },
  ...
  last_slide: function () {
    if (!w3cSlidy.view_all) {
      ...
      w3cSlidy.last_shown = null // revealNextItem(lastShown);

      if (w3cSlidy.last_shown == null &&
        w3cSlidy.slide_number < w3cSlidy.slides.length - 1) {
        slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)
        w3cSlidy.slide_number = w3cSlidy.slides.length - 1
  ...
  set_focus: function (element) {
    if (element) {
      ...
      w3cSlidy.help_anchor.focus()

      setTimeout(function () {
        w3cSlidy.help_anchor.blur()
      }, 1)
    }
  },
  ...
  sync_background: function (slide) {
    ...
    if (slide.currentStyle) {
      ...
      const styles = document.defaultView.getComputedStyle(slide, null)
    ...
    if (bgColor === 'transparent' ||
      ...
      const slideClass = this.get_class_list(slide)

      for (let i = 0; i < this.backgrounds.length; i++) {
        background = this.backgrounds[i]

        const bgClass = this.get_class_list(background)
  ...
  matching_background: function (slideClass, bgClass) {
    ...
    for (let i = count = 0; i < result.length; i++) {
    ...
    if (count === 0) { return true }// default match
    ...
    for (let i = count = 0; i < result.length; i++) {
      ...
      if (this.has_token(bgClass, result[i])) { return true }
    }
  ...
  resized: function () {
    ...
    // IE fires on resize even when only font size is changed!
    // so we do a check to avoid blocking < and > actions
    if (width !== w3cSlidy.last_width || height !== w3cSlidy.last_height) {
      if (width >= 1100) {
        w3cSlidy.size_index = 5 // 4
      } else if (width >= 1000) {
        w3cSlidy.size_index = 4 // 3
      } else if (width >= 800) {
        w3cSlidy.size_index = 3 // 2
      } else if (width >= 600) {
        w3cSlidy.size_index = 2 // 1
      } else if (width) {
        w3cSlidy.size_index = 0
      }
      ...
      if (w3cSlidy.size_index + w3cSlidy.size_adjustment >= 0 &&
        w3cSlidy.size_index + w3cSlidy.size_adjustment < w3cSlidy.sizes.length) {
        w3cSlidy.size_index = w3cSlidy.size_index + w3cSlidy.size_adjustment
      }

      // enables cross browser use of relative width/height
      // on object elements for use with SVG and Flash media
      w3cSlidy.adjust_object_dimensions(width, height)

      if (document.body.style.fontSize !== w3cSlidy.sizes[w3cSlidy.size_index]) {
        document.body.style.fontSize = w3cSlidy.sizes[w3cSlidy.size_index]
      }

      w3cSlidy.last_width = width
      w3cSlidy.last_height = height

      // force reflow to work around Mozilla bug
      if (w3cSlidy.ns_pos) {
        const slide = w3cSlidy.slides[w3cSlidy.slide_number]
        w3cSlidy.hide_slide(slide)
        w3cSlidy.show_slide(slide)
      }

      // force correct positioning of toolbar
      w3cSlidy.refresh_toolbar(200)
    }
  },

  scrolled: function () {
    if (w3cSlidy.toolbar && !w3cSlidy.ns_pos && !w3cSlidy.ie7) {
      w3cSlidy.hack_offset = w3cSlidy.scroll_x_offset()
      // hide toolbar
      w3cSlidy.toolbar.style.display = 'none'

      // make it reappear later
      if (w3cSlidy.scrollhack === 0 && !w3cSlidy.view_all) {
        setTimeout(function () { w3cSlidy.show_toolbar() }, 1000)
        w3cSlidy.scrollhack = 1
      }
    }
  },

  hide_toolbar: function () {
    w3cSlidy.add_class(w3cSlidy.toolbar, 'hidden')
    window.focus()
  },

  // used to ensure IE refreshes toolbar in correct position
  refresh_toolbar: function (interval) {
    if (!w3cSlidy.ns_pos && !w3cSlidy.ie7) {
      w3cSlidy.hide_toolbar()
      setTimeout(function () { w3cSlidy.show_toolbar() }, interval)
    }
  },

  // restores toolbar after short delay
  show_toolbar: function () {
    if (w3cSlidy.want_toolbar) {
      w3cSlidy.toolbar.style.display = 'block'

      if (!w3cSlidy.ns_pos) {
        // adjust position to allow for scrolling
        const xoffset = w3cSlidy.scroll_x_offset()
        w3cSlidy.toolbar.style.left = xoffset
        w3cSlidy.toolbar.style.right = xoffset
        ...
        w3cSlidy.toolbar.style.bottom = 0 // bottom;
      }

      w3cSlidy.remove_class(w3cSlidy.toolbar, 'hidden')
    }

    w3cSlidy.scrollhack = 0
  ...
  toggle_toolbar: function () {
    if (!w3cSlidy.view_all) {
      if (w3cSlidy.has_class(w3cSlidy.toolbar, 'hidden')) {
        w3cSlidy.remove_class(w3cSlidy.toolbar, 'hidden')
        w3cSlidy.want_toolbar = 1
      } else {
        w3cSlidy.add_class(w3cSlidy.toolbar, 'hidden')
        w3cSlidy.want_toolbar = 0
      }
    }
  },
  ...
  smaller: function () {
    ...
    w3cSlidy.hide_slide(slide)
    w3cSlidy.show_slide(slide)
    setTimeout(function () { w3cSlidy.show_toolbar() }, 50)
  },

  bigger: function () {
    if (w3cSlidy.size_index < w3cSlidy.sizes.length - 1) {
      ++w3cSlidy.size_index
    }
  ...
  adjust_object_dimensions: function (width, height) {
    for (let i = 0; i < w3cSlidy.objects.length; i++) {
      ...
      if (mimeType === 'image/svg+xml' || mimeType === 'application/x-shockwave-flash') {
        ...
        if (obj.initialWidth && obj.initialWidth.charAt(obj.initialWidth.length - 1) === '%') {
          const w = parseInt(obj.initialWidth.slice(0, obj.initialWidth.length - 1))
          const newW = width * (w / 100.0)
          obj.setAttribute('width', newW)
        }

        if (obj.initialHeight &&
            obj.initialHeight.charAt(obj.initialHeight.length - 1) === '%') {
          const h = parseInt(obj.initialHeight.slice(0, obj.initialHeight.length - 1))
          const newH = height * (h / 100.0)
  ...
  key_press: function (event) {
    ...
    if (!w3cSlidy.key_wanted) { return w3cSlidy.cancel(event) }
  ...
  key_down: function (event) {
    ...
    if (!w3cSlidy.slidy_chrome(target) &&
        w3cSlidy.special_element(target)) {
      return true
    }
    ...
    if (w3cSlidy.is_shown_toc() && key !== 9 && key !== 16 && key !== 38 && key !== 40) {
      w3cSlidy.hide_table_of_contents(true)
    ...
    if (key === 34) { // Page Down
      if (w3cSlidy.view_all) { return true }

      w3cSlidy.next_slide(false)

      return w3cSlidy.cancel(event)
    } else if (key === 33) { // Page Up
      if (w3cSlidy.view_all) { return true }

      w3cSlidy.previous_slide(false)
      ...
      w3cSlidy.previous_slide(!event.shiftKey)

      return w3cSlidy.cancel(event)
    } else if (key === 36) { // Home
      w3cSlidy.first_slide()
      ...
      w3cSlidy.next_slide(!event.shiftKey)

      return w3cSlidy.cancel(event)
    } else if (key === 13) { // Enter
      if (w3cSlidy.outline) {
        if (w3cSlidy.outline.visible) {
          w3cSlidy.fold(w3cSlidy.outline)
        } else {
          w3cSlidy.unfold(w3cSlidy.outline)
        }
      ...
      w3cSlidy.toggle_toolbar()

      return w3cSlidy.cancel(event)
    } else if (key === 65) { // A for toggle view single/all slides
      w3cSlidy.toggle_view()

      return w3cSlidy.cancel(event)
    } else if (key === 75) { // toggle action of left click for next page
      w3cSlidy.mouse_click_enabled = !w3cSlidy.mouse_click_enabled

      const alertMsg = (w3cSlidy.mouse_click_enabled
        ? 'enabled'
        : 'disabled') + ' mouse click advance'

      alert(w3cSlidy.localize(alertMsg))
      ...
      window.location = w3cSlidy.help_page
  ...
  add_class: function (element, name) {
    if (!this.has_class(element, name)) {
      if (typeof element.className !== 'undefined') {
        ...
        let clsval = element.getAttribute('class')
  ...
  next_incremental_item: function (node) {
    const br = this.is_xhtml ? 'br' : 'BR'
    const slide = w3cSlidy.slides[w3cSlidy.slide_number]

    for (; ;) {
      node = w3cSlidy.next_node(slide, node)
      ...
      if (node.nodeType === 1) { // ELEMENT
        ...
        if (w3cSlidy.has_class(node, 'incremental') &&
            w3cSlidy.okay_for_incremental(node.nodeName)) {
          return node
        }

        if (w3cSlidy.has_class(node.parentNode, 'incremental') &&
            !w3cSlidy.has_class(node, 'non-incremental')) {
          return node
        }
      }
    }
  ...
  previous_incremental_item: function (node) {
    ...
    for (; ;) {
      node = w3cSlidy.previous_node(slide, node)
  ...
  set_visibility_all_incremental: function (value) {
    let node = this.next_incremental_item(null)

    if (value === 'hidden') {
      while (node) {
        w3cSlidy.add_class(node, 'invisible')
        node = w3cSlidy.next_incremental_item(node)
      }
    } else { // value == "visible"
      while (node) {
        w3cSlidy.remove_class(node, 'invisible')
        node = w3cSlidy.next_incremental_item(node)
      }
    }
  },
  ...
  mouse_button_up: function (_) {
    w3cSlidy.selected_text_len = w3cSlidy.get_selected_text().length
  },

  mouse_button_down: function (e) {
    w3cSlidy.selected_text_len = w3cSlidy.get_selected_text().length
    w3cSlidy.mouse_x = e.clientX
    w3cSlidy.mouse_y = e.clientY
  },

  // right mouse button click is reserved for context menus
  // it is more reliable to detect 1 than leftclick
  mouse_button_click: function (e) {
    if (!e) {
      e = window.event
    }

    if (Math.abs(e.clientX - w3cSlidy.mouse_x) +
        Math.abs(e.clientY - w3cSlidy.mouse_y) > 10) {
      return true
    }

    if (w3cSlidy.selected_text_len > 0) { return true }
    ...
    if (e.which) { // all browsers except IE
      this.leftclick = (e.which === 1)
      this.middleclick = (e.which === 2)
      this.rightclick = (e.which === 3)
    } else if (e.button) {
    ...
    w3cSlidy.hide_table_of_contents(false)

    // check if target is something that probably want's clicks
    // e.g. a, embed, object, input, textarea, select, option
    // var tag = target.nodeName.toLowerCase();

    if (w3cSlidy.mouse_click_enabled && this.leftclick &&
      !w3cSlidy.special_element(target) &&
      !target.onclick) {
      w3cSlidy.next_slide(true)
      w3cSlidy.stop_propagation(e)
  ...
  special_element: function (element) {
    if (this.has_class(element, 'non-interactive')) { return false }
  ...
  localize: function (src) {
    ...
    let s = w3cSlidy.strings[w3cSlidy.lang]
    let lookup = w3cSlidy.strings[w3cSlidy.lang]
    ...
    const lg = w3cSlidy.lang.split('-')

    if (lg.length > 1) {
      lookup = w3cSlidy.strings[lg[0]]
  ...
  init_localization: function () {
    const i18n = w3cSlidy
    const helpText = w3cSlidy.help_text
    ...
    i18n.strings_es[helpText] = i18n.help_es
    i18n.strings_ca[helpText] = i18n.help_ca
    i18n.strings_cs[helpText] = i18n.help_cs
    i18n.strings_nl[helpText] = i18n.help_nl
    i18n.strings_de[helpText] = i18n.help_de
    i18n.strings_pl[helpText] = i18n.help_pl
    i18n.strings_fr[helpText] = i18n.help_fr
    i18n.strings_hu[helpText] = i18n.help_hu
    i18n.strings_it[helpText] = i18n.help_it
    i18n.strings_el[helpText] = i18n.help_el
    i18n.strings_ja[helpText] = i18n.help_ja
    i18n.strings_zh[helpText] = i18n.help_zh
    i18n.strings_ru[helpText] = i18n.help_ru
    i18n.strings_sv[helpText] = i18n.help_sv

    w3cSlidy.lang = document.body.parentNode.getAttribute('lang')

    if (!w3cSlidy.lang) {
      w3cSlidy.lang = document.body.parentNode.getAttribute('xml:lang')
    }
...
setTimeout(w3cSlidy.hide_slides, 50)
